...
exports.crawl_bdfb = function(){
    ...
    setInterval(function(){
        ...
        let options = {
            method:"POST",
            //json: true,
            url:"http://pkulaw.cn/case/Search/Record",
            timeout:10000,
            headers:{
        ...
        request(options,function(error,response,body){
            if(error){
                console.log('crawl_bdfb----->error',error)
                return false
            }
            if(!error && response.statusCode == 200){
                ...
                async.eachLimit( $('a.title'),1,function(item,cbb){
                    console.log('item--->',$(item).text())
                    console.log()
                    let msalsave = new msal({
                        bdfb_title : excludeSpecial($(item).text()),
                        bdfb_alink : 'http://www.pkulaw.cn' + excludeSpecial($(item).attr('href'))
                    })
                    msalsave.save(function(saveerr){
                        if(saveerr){
                            console.log('------ save title and link err ------')
                            cbb(saveerr)
                        }
                        console.log('------ save title and link 成功 ------')
                        console.log()
                        cbb()
                    })
                },function(err){
                    if(err){
                        console.log('----- 插入文档名和链接 err ------',err)
                        return false
                    }
                    page = page + 1
                    console.log('----- 插入文档名和链接成功 抓取下一页------',page)
                    console.log()
                    return false
                })
            }
